<div class="container">

    <h1>Seguro</h1>

    <div class="col-xs-12">
        <h2>Responsabilidad Civil - (Seguro de RC).</h2>
        <p>Es obligatorio según ley de tránsito Nº24.499 y resoluciones de la SSN Nº 34.225 y Nº 27.033.</p>

        <p>Contamos con el Respaldo, la Trayectoria y la Seguridad de las Principales Compañías de Seguros del Mercado. Te aseguramos la unidad al instante para que puedas disfrutarla con total tranquilidad. asesorate con nosotros y asegurala ya !</p>


    </div>

    <h1>Cobertura</h1>

    <div class="col-xs-12">
        <h2>RC (Responsabilidad Civil) </h2>
        <ul>
            <li>
                Daños Corporales a Personas No Transportadas
            </li>
            <li>
                Daños Materiales a Cosas de Terceros.
            </li>
            <li>
                Daños Corporales a Personas Transportadas (Acompañante)
            </li>
            <li>
                Nota: Cada una de las compañías de seguros tiene diferentes valores de cobertura en $.
            </li>
        </ul>

    </div>

    <div class="col-xs-12">
        <h2>RC + Robo (Responsabilidad Civil + Incendio + Robo + Auxilio Mecanico)  </h2>
        <ul>
            <li>
                Todos los Puntos Referidos en Cobertura de RC
            </li>
            <li>
                Indemnización por Robo Total del Motovehículo por el valor de la suma asegurada.
            </li>
            <li>
                Auxilio mecanico hasta 500km.
            </li>
        </ul>

    </div>

</div>

</div>